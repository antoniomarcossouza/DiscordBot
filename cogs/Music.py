...
class Music(commands.Cog):
    ...
    async def volume(self, ctx, volume: int):
        if ctx.voice_client is None:
            return await ctx.send("Não estou conectado à um canal de voz.")
    ...
    async def pause(self, ctx):
        ...
        await ctx.channel.send("Reprodução pausada!")

    # Bot resumes the reproduction
    @commands.command()
    async def resume(self, ctx):
        await ctx.voice_client.resume()
        await ctx.channel.send("Reprodução resumida!")
    ...
    async def ensure_voice(self, ctx):
        if ctx.voice_client is None:
            if ctx.author.voice:
                ...
                await ctx.send("Você não está conectado à um canal de voz!")
                raise commands.CommandError(
                    "Autor não está conectado à um canal de voz."
                )
